...
try {
	...
	// 当前标签下的博客信息
	if (username.equals("admin") || username.equals("tourist"))
		sql_2 = String.format("select blogID,title,content,releaseTime,backgroundImg,catName from blog natural join users natural join cat where catName = '%s'", catName);
	else
	sql_2 = String.format("select blogID,title,content,releaseTime,backgroundImg,catName from blog natural join users natural join cat natural join tag where userID = %s and catName = '%s'", userID, catName);
	ResultSet rs_2 = stmt.executeQuery(sql_2);//执行查询，返回结果集
	while(rs_2.next()) { //把游标(cursor)移至第一个或下一个记录
	...
	%>
                                        ...
                                        <nav id="nav_header" class="bg-color nav-transparent">
                                            <div id="navContainer" class="nav-wrapper container">
                                                <div class="right brand-logo">
                                                    <!-- <a href="home.jsp" class="waves-effect waves-light"> <img src="medias/imgs/sysu.ico" class="logo-img" alt="LOGO">
                                                    </a> -->
                                    ...
                                    <div class="container">
                                        <div class="row">
                                            <div class="col s10 offset-s1 m8 offset-m2 l8 offset-l2">
                                                <div class="brand">
                                                    <div class="title center-align">博客分类</div>
                                                    <div class="description center-align">
                                                        Categorize Blog Article
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
	                        ...
	                        <span class="tag-length"><%out.print(catcount.get(i)); %></span>
								...
								<%out.print(articles.get(i).get("catName")); %> -->
                                                            ...
                                                            </a> <img src="./medias/trash.png" onclick=delBlog(<%out.print(articles.get(i).get( "blogID")); %>) style="float: right;position: relative; right: 10px; width: 20px; height: 20px; margin-bottom: 10px; cursor: pointer;">
                                                        </div>
                                                    </div>
                                                </div>
                                                <%}%>
